...
- **Post-processing** — built-in bloom/glow with configurable threshold and radius
...
| Key               | Default | Description                               |
| ----------------- | ------- | ----------------------------------------- |
| `bloom_strength`  | `0.0`   | Bloom intensity (0 = off, 1 = heavy glow) |
| `bloom_threshold` | `0.6`   | HDR brightness above which bloom triggers |
| `bloom_radius`    | `0.02`  | Blur radius as fraction of image diagonal |
...
When `exr_output` is set, the renderer writes a multi-layer float32 OpenEXR file:
...
These separate layers allow independent colour grading, sky replacement, and compositing in software like DaVinci Resolve, Nuke, or After Effects.
...
Standalone post-processing tools live in the `tools/` directory. See [tools/README.md](tools/README.md) for details.

- **bhrt-bloom** — Reads EXR output and applies configurable bloom, exposure, tonemapping, and sky brightness adjustment. Outputs to TGA, HDR, or EXR.
...
├── src/                    # C++ source files
│   ├── main.cpp            # Render loop, post-processing, output writers
...
│   └── bloom.cpp           # EXR bloom/grading tool
